...
export default function RealEstatePage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans overflow-x-hidden">
      ...
      <footer className="bg-[#080807] border-t border-[#C8A951]/10">
        <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          {/* Brand */}
          <div className="lg:col-span-1">
            <div className="mb-4">
              <div className="text-2xl font-light tracking-[0.25em] text-[#C8A951] mb-1" style={{ fontFamily: "Georgia, serif" }}>LUXE <span className="text-white/30 text-lg">PROPERTIES</span></div>
            </div>
            <p className="text-gray-500 text-sm leading-relaxed mb-6">Handpicked residences for the discerning few. From Manhattan penthouses to private island escapes. The world's most exclusive addresses.</p>
            <div className="flex gap-3">
              {[
                ...
                <button key={i} className="w-9 h-9 border border-[#C8A951]/20 hover:border-[#C8A951] hover:text-[#C8A951] text-gray-600 flex items-center justify-center rounded transition-all duration-300">
                  {icon}
                </button>
              ))}
            </div>
          ...
          {/* Properties */}
          <div>
            <h4 className="text-[#C8A951] tracking-[0.3em] uppercase text-xs mb-6" style={{ fontFamily: "Georgia, serif" }}>Properties</h4>
            <ul className="space-y-3">
              {["All Residences", "Houses", "Villas", "Penthouses", "Townhouses", "New Listings"].map((t, i) => (
                <li key={t}>
                  <button onClick={() => setTypeFilter(i === 0 ? "All" : t.replace("s", "") === "Villas" ? "Villa" : t === "Houses" ? "House" : t === "Penthouses" ? "Penthouse" : t === "Townhouses" ? "Townhouse" : "All")}
                    className="text-gray-500 hover:text-[#C8A951] transition-colors text-sm tracking-wide">{t}</button>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* Services */}
          <div>
            <h4 className="text-[#C8A951] tracking-[0.3em] uppercase text-xs mb-6" style={{ fontFamily: "Georgia, serif" }}>Services</h4>
            <ul className="space-y-3">
              {["Buying", "Selling", "Rental Portfolio", "Property Valuation", "Investment Advisory", "Relocation"].map(s => (
                <li key={s}><button className="text-gray-500 hover:text-[#C8A951] transition-colors text-sm tracking-wide">{s}</button></li>
              ))}
            </ul>
          </div>
          ...
          {/* Contact */}
          <div>
            <h4 className="text-[#C8A951] tracking-[0.3em] uppercase text-xs mb-6" style={{ fontFamily: "Georgia, serif" }}>Contact</h4>
            <ul className="space-y-4 text-sm mb-6">
              {[
                { icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>, text: "One Central Park, New York, NY 10024" },
                ...
                { icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>, text: "enquiries@luxeproperties.com" },
              ].map(({ icon, text }, i) => (
                <li key={i} className="flex gap-3 items-start">
                  <span className="text-[#C8A951]/60 mt-0.5 flex-shrink-0">{icon}</span>
                  <span className="text-gray-500">{text}</span>
                </li>
              ))}
            </ul>
            <button onClick={() => setContactModal(true)}
              className="w-full py-3 border border-[#C8A951]/40 text-[#C8A951] hover:bg-[#C8A951] hover:text-black transition-all duration-300 text-xs tracking-[0.25em] uppercase">
              Contact an Agent
            </button>
          </div>
        </div>
        ...
        <div className="border-t border-[#C8A951]/10">
          <div className="max-w-7xl mx-auto px-6 py-5 flex flex-col md:flex-row justify-between items-center gap-3">
            <p className="text-gray-700 text-xs tracking-wide">&copy; {new Date().getFullYear()} Luxe Properties. All rights reserved.</p>
